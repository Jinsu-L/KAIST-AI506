...
class Task1Net(nn.Module):
    def __init__(self, n_itemset, n_users, user_embedding, item_embedding, style_embedding):
        ...
        self.fc = nn.Linear(64 * 2 + 256 + 2, 128)
        ...
        self.pop_fc = nn.Linear(4, 2)

        # self.pop_out = nn.Linear(2, 1)

        self.bn1 = torch.nn.BatchNorm1d(64 * 2 + 256 + 2)
    ...
    def forward(self, user, itemset, item_list=[], length_list=[], pop=[]):
        ...
        h = F.relu(torch.concat([user_embeds, itemset_embeds, itemset_item_embeds, pop_score], 1))  # batch, dim*3
        h = self.bn1(h)
        h = F.relu(self.fc(h))
        h = self.bn2(h)
